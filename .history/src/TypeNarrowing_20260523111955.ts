...
function serveOrder(item: ChaiOrder | string) {
    if (isChaiOrder(item)) {
        return `Serving ${item.sugar} with ${item.type}`
    }
    return `Serving custom chai is ${item}`
}

type MasalaChai = {type: "masala",spiceLevel:number}
type GingerChai = {type: "ginger",spiceLevel:number}
type ElaichiChai = { type: "eliachi", spiceLevel: number }
...
function brew(order: MasalaChai | GingerChai | ElaichiChai) {
    if ("spiceLevel" in order) {
        return `order is for MasalaChai`
    }
}

function isStringArray(arr:unknown):arr is Str